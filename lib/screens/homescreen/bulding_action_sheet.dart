...
Future<void> showBuildingActions(
  BuildContext ctx,
  Building b, {
  required VoidCallback onUpgradeOk,    // ← nuevo
  TrainingNav? onTrain,
}) => showDialog(
      context: ctx,
      ...
      builder: (_) => _ActionDialog(
        building: b,
        onTrain: onTrain,
        onUpgradeOk: onUpgradeOk,        // pasa abajo
      ),
    );
...
final double maxW = (size.width * .60).clamp(0.0, 500.0);  // ← FIX

    return Dialog(
      backgroundColor: const Color(0xff202020),
      insetPadding: EdgeInsets.symmetric(
          horizontal: size.width * .2, vertical: 40), // centrado
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: Text(
                '${_names[b.id] ?? b.id}  Lv${b.level} → Lv$nextLv',
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: _loading
                          ...
                          : () async {
                              ...
                              final resp =
                                  await api.startConstruction(b.id, nextLv);
                              if (!mounted) return;
                              setState(() => _loading = false);
                              if (resp['ok'] == true) {
                                  widget.onUpgradeOk();                 // ← avisa a HomeScreen

await context.read<ApiService>().fetchUserStats(); // refresca Provider
await context.read<ApiService>().fetchBuildQueue(); // refresca cola
Navigator.pop(context);
                  ...
                  if ((b.id == 'barracks' || b.id == 'coliseo') && widget.onTrain != null)
...
int? _production(String id, int lv) =>
    (id == 'farm' || id == 'lumbermill' || id == 'stonemine')
        ? lv * 50
        : null;
